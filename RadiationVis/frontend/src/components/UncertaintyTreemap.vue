  ...
  methods: {
    ...
    drawTreemap1() {
      ...
      function mousemove(d) {
        mytooltip
            .html(`SSs: ${d.data.static.map(s => s.name.substring(1)).toString() || 'null'} <br/>MSs: ${d.data.mobile.map(s => s.name.substring(1)).toString() || 'null'}<br/>standard deviation: ${d.data.std.toFixed(2)}`)
            .style('left', () => {
    ...
    drawTreemap2() {
      ...
      function mousemove(d) {
        mytooltip
            .html(`standard deviation: ${d.data.std.toFixed(2)}`)
            .style('left', () => {
              if(d3.event.offsetX + 150 > _this.svgWidth) {
                return  (d3.event.offsetX - 150) + 'px'
              } else {
