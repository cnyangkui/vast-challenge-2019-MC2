  ...
  methods: {
    ...
    drawPaths (g, data, x, y, color) {
      ...
      function mouseover() {
        ...
        mytooltip
            .html(`time: ${timeFormat(d.time)} <br/>average radiation readings: ${d.avg.toFixed(2)}`)
            .style('left', () => {
    ...
    drawChart() {
      ...
      let max1 = d3.max(this.originData.data.staticData, d => d.avg);
      let max2 = d3.max(this.originData.data.mobileData, d => d.avg);
      let max;
