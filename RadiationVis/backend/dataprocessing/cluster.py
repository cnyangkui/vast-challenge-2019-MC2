...
def calCorrlelation(begintime, endtime):
    ...
    feature_matrix = Distance_Metric_Cos(mn, sensor_len)
    ...
    for i in range(len(class_type)):
        tree["name"] = 'cluster' + str(class_type[i])
        tree["children"] = {"class": class_type[i],"children":[]}
    for i in range(len(cluster_label)):
        tree["children"]["children"].append({"mean":col_mean[i], "std":col_std[i]})
    print(tree)
    return tree
...
if __name__ == '__main__':
    ...
    calCorrlelation('2020-04-07 00:00:00', '2020-04-08 12:00:00')